...
from ..schemas import LoginFormData, CreateAccountFormData
...
def login(
    data: Annotated[LoginFormData, Form()], conn: sqlite3.Connection = Depends(get_db)
):
    ...
    token = create_token(
        id=str(user["id"]), username=user["username"], isadmin=user["isadmin"]
    )
...
def signup(
    data: Annotated[CreateAccountFormData, Form()],
    ...
    if (
        ...
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENITITY,
            detail="Data provided is not valid!!!",
        )
    ...
    token = create_token(
        id=str(newuser["id"]), username=newuser["username"], isadmin=newuser["isadmin"]
    )
    response = RedirectResponse(url="/frontend", status_code=status.HTTP_303_SEE_OTHER)
    response.set_cookie(key="access-token", value=token)
    return response
